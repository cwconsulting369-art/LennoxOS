...
import { TrendingUp, Wifi, WifiOff, ExternalLink, Map, LayoutDashboard } from 'lucide-react';
import { RoadmapPanel } from '../components/RoadmapPanel';

const ROADMAP_PATH = '/home/carlos/personal-os/01-business/aevum/ROADMAP.md';

export default function AevumOS() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const res = await fetch('https://aevum-os.lennoxos.com', { method: 'HEAD', signal: AbortSignal.timeout(4000) });
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-os-cyan/10">
            <TrendingUp size={16} className="text-os-cyan" />
          </div>
          <div>
            <h1 className="text-sm font-semibold text-os-text">AEVUM OS</h1>
            <p className="text-[10px] text-os-muted">aevum-os.lennoxos.com</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <a href="https://aevum-os.lennoxos.com" target="_blank" rel="noreferrer"
      ...
      <div className="flex-1 overflow-hidden">
        {tab === 'dashboard' && (
          <div className="h-full rounded-xl border border-os-border overflow-hidden">
            <iframe src="https://aevum-os.lennoxos.com" className="h-full w-full border-0"
