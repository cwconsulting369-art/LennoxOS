...
export default function KetolabsOS() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const res = await fetch('https://ketolabs.lennoxos.com', { method: 'HEAD', signal: AbortSignal.timeout(4000) });
    ...
    <div className="flex h-[calc(100vh-0px)] flex-col p-6 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-os-green/10">
            <FlaskConical size={16} className="text-os-green" />
          </div>
          <div>
            <h1 className="text-sm font-semibold text-os-text">Ketolabs OS</h1>
            <p className="text-[10px] text-os-muted">ketolabs.lennoxos.com</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <a href="https://ketolabs.lennoxos.com" target="_blank" rel="noreferrer"
